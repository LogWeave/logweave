...
if [[ "$OSTYPE" == "darwin"* ]]; then
  ...
  sed -i '' "s|LOGWEAVE_CLICKHOUSE_PASSWORD=.*|LOGWEAVE_CLICKHOUSE_PASSWORD=${CH_PASSWORD}|" "$ENV_FILE"
  sed -i '' "s|CLICKHOUSE_PASSWORD_SHA256=.*|CLICKHOUSE_PASSWORD_SHA256=${CH_PASSWORD_SHA256}|" "$ENV_FILE"
else
  # Linux/Git Bash sed
  sed -i "s|LOGWEAVE_API_KEYS=.*|LOGWEAVE_API_KEYS={\"${API_KEY}\":\"default\"}|" "$ENV_FILE"
  sed -i "s|LOGWEAVE_ENCRYPTION_KEY=.*|LOGWEAVE_ENCRYPTION_KEY=${ENCRYPTION_KEY}|" "$ENV_FILE"
...
echo "    # On first boot a random admin password is printed once — grab it:"
echo "    docker compose -f docker-compose.prod.yml logs api | grep -A3 'LOGWEAVE BOOTSTRAP'"
echo "    open http://localhost:3000 (login: admin / <that password>)"
echo ""
