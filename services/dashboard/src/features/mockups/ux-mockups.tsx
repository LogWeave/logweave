...
const PATTERNS = [
  ...
  {
    text: 'User <*> authenticated successfully via <*> from <*>',
    service: 'auth-api',
    count: 31,
    errors: 0,
    isNew: true,
  ...
  {
    text: 'Payment processed: <*> <*> USD via <*> <*> <*>',
  ...
  {
    text: 'Email sent: <*> to <EMAIL> via <*> in <*>',
  ...
  {
    text: 'Token refreshed for session <UUID> — new TTL <*>',
    service: 'auth-api',
    count: 15,
    errors: 1,
    isNew: false,
  ...
  {
    text: 'Webhook delivered to <*> — status <*> in <*>',
    service: 'payments-api',
    count: 12,
    errors: 0,
    isNew: false,
  ...
  {
    text: 'Rate limit exceeded: <EMAIL> — <*> capped at <*>',
    service: 'notifications-svc',
    count: 9,
    errors: 3,
    isNew: true,
    ...
    <div className="space-y-3">
        ...
        {HERO_KPIS.map((kpi) => (
          <Card key={kpi.label}>
            <div className="flex items-center gap-3">
              <div
                className={`h-10 w-10 rounded-lg flex items-center justify-center shrink-0 ${kpi.variant === 'danger' ? 'bg-red-500/10 text-danger' : kpi.variant === 'warning' ? 'bg-amber-500/10 text-warning' : 'bg-brand-500/10 text-brand-400'}`}
              >
                <kpi.icon size={20} />
              </div>
              <div>
      ...
      <div className="flex items-center gap-3">
        {SECONDARY_KPIS.map((kpi) => (
          <div
            key={kpi.label}
            className="flex items-center gap-2 px-3 py-2 rounded-lg bg-surface-base border border-border-subtle"
          >
      ...
      {SERVICES.map((svc) => (
        <Card key={svc.name} variant="interactive">
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              className="shrink-0 h-7 w-7 rounded-md flex items-center justify-center text-text-muted hover:text-brand-400"
            >
              <Bell size={14} />
            </button>
          </div>
          ...
          <Card key={svc.name} variant="interactive">
            <div className="text-center">
              ...
              <div className="flex items-center justify-center gap-3 mt-2">
                ...
                <div>
                  <p className="text-lg font-bold font-mono tabular-nums text-danger">
                    {svc.errorRate}%
                  </p>
              ...
              {svc.newPatterns > 0 && (
                <Badge variant="new" className="mt-2">
                  {svc.newPatterns} new
                </Badge>
              )}
            </div>
    ...
    <Card>
      ...
      <CardContent className="p-0">
        {SERVICES.map((svc, i) => (
          <div
            key={svc.name}
            className={`flex items-center gap-3 px-4 py-3 hover:bg-surface-elevated transition-colors ${i < SERVICES.length - 1 ? 'border-b border-border-subtle/50' : ''}`}
          >
            <div
              className={`h-2 w-2 rounded-full shrink-0 ${svc.errorRate > 2 ? 'bg-danger' : 'bg-success'}`}
            />
            <span className="text-sm font-medium text-text-primary flex-1">{svc.name}</span>
            <span className="text-xs text-text-muted font-mono tabular-nums">
              {svc.events.toLocaleString()}
            </span>
            <span className="text-xs text-danger font-mono tabular-nums w-12 text-right">
              {svc.errorRate}%
            </span>
            {svc.newPatterns > 0 && (
              <Badge variant="new" className="shrink-0">
    ...
    <Card>
      ...
      <CardContent className="space-y-2">
        {PATTERNS.slice(0, 5).map((p) => (
          <div
            key={p.text}
            className="p-3 rounded-lg bg-surface-base hover:bg-surface-elevated transition-colors cursor-pointer"
          >
            <div className="flex items-start gap-2">
              <code className="text-xs font-mono text-text-primary flex-1 leading-relaxed">
                {p.text}
              </code>
              {p.isNew && (
                <Badge variant="new" className="shrink-0">
    ...
    <Card>
      ...
      <CardContent className="p-0">
        {PATTERNS.slice(0, 6).map((p, i) => (
          <div
            key={p.text}
            className={`flex items-center gap-3 px-4 py-2.5 hover:bg-surface-elevated transition-colors cursor-pointer ${i < 5 ? 'border-b border-border-subtle/50' : ''}`}
          >
            ...
            {p.errors > 0 && (
              <span className="text-xs font-mono tabular-nums text-danger shrink-0">
                {p.errors}
              </span>
            )}
            {p.isNew && (
              <Badge variant="new" className="shrink-0 text-[9px]">
                NEW
              </Badge>
            )}
          </div>
            ...
            <div key={service}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] text-text-muted">
                  {servicePatterns.length} patterns
                </span>
              </div>
              <div className="space-y-1 pl-4 border-l border-border-subtle">
                {servicePatterns.map((p) => (
                  <div
                    key={p.text}
                    className="flex items-center gap-2 py-1 hover:bg-surface-elevated rounded px-2 -mx-2 cursor-pointer"
                  >
    ...
    <div className="space-y-12 max-w-5xl pb-20">
      <div>
        ...
        <p className="text-sm text-text-muted">
          Compare options side by side. Resize your browser to see responsive behavior.
        </p>
      </div>
      ...
      <section>
        <SectionHeader
          title="KPI Strip"
          description="Option A: Priority tier (hero + pills). Option B: Simple grid. Option C: Single card with dividers."
        />
        <div className="space-y-8">
          ...
          <div>
            <p className="text-xs text-brand-400 font-medium mb-2">
              Option C — Unified Card with Dividers
            </p>
            <KpiOptionC />
          </div>
        </div>
      ...
      <section>
        <SectionHeader
          title="Service Cards"
          description="Option A: Vertical stack (current style, fixed). Option B: 2-column grid. Option C: Compact table rows."
        />
      ...
      <section>
        <SectionHeader
          title="Patterns"
          description="Option A: Cards with metadata below. Option B: Compact list (like current but responsive). Option C: Grouped by service."
        />
        <div className="space-y-8">
          <div>
