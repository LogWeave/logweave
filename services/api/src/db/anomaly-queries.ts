...
SELECT
  ...
  countMerge(occurrence_count) / (uniq(toDate(interval_start)) * {buckets_per_hour:UInt32}) AS avg_count_per_interval
 ...
 *
 * Returns the true per-interval rate: total occurrences divided by the number
 * of 5-min buckets the hour-of-day *could* have had over the observed window
 * (distinct days × 12), so silent buckets count as the zeros they are.
 * Dividing by `uniq(interval_start)` instead (buckets that actually fired)
 * would overstate "normal" for sparse/bursty templates and suppress real
 * spikes. See ADR-014.
 *
