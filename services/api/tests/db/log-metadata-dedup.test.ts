...
describe('log_metadata ReplacingMergeTree(event_id) dedup', () => {
  ...
  // NOTE: in-place migration of a legacy MergeTree log_metadata was removed —
  // initSchema now refuses to start on an engine mismatch rather than dropping
  // the table. That guard (assertLogMetadataEngine) is covered in
